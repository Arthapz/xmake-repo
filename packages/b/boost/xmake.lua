package("boost")
    ...
    on_install("macosx", "linux", "windows", "bsd", "mingw", "cross", function (package)
        ...
        if version:ge("1.84.0") then
            ...
            for _, libname in ipairs(libnames) do
                local enabled = package:config("all") or package:config(libname)
                if enabled then
                    enabled_libraries = enabled_libraries or {}
                    table.insert(enabled_libraries, libname)
                end
            ...
            table.insert(configs, "-DBOOST_INCLUDE_LIBRARIES=" .. (enabled_libraries and table.concat(enabled_libraries, ";") or ""))
